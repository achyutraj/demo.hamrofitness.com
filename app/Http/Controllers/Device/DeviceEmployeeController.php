...
class DeviceEmployeeController extends GymAdminBaseController
{
    ...
    public function index(){
        ...
        $this->data['title'] = "Employee Biometrics";
        $this->data['devices'] = Device::where('device_status', 1)->where('detail_id',$this->data['user']->detail_id)
                ->get();
        if( $this->data['devices']->count() > 1){
            $this->data['clients'] = GymClient::GetEmployeeFromClients($this->data['user']->detail_id)->has('devices')->get();
            return view('devices.employ-biometrics.multi-device-index', $this->data);
        }else{
            return view('devices.employ-biometrics.index', $this->data);
        }
    ...
    public function ajax_create(){
        ...
        $gym_clients = GymClient::GetEmployeeFromClients($this->data['user']->detail_id)->has('devices')->get();
