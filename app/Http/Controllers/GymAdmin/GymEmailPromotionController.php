...
class GymEmailPromotionController extends GymAdminBaseController
{
    ...
    public function ajaxCreate()
    {
        ...
        return Datatables::of($promotions)
            ->addColumn(
                ...
                if ($row->status == 'sent') {
                    return "<a class='btn green btn-xs' href='" . route("gym-admin.email-promotion.edit-campaign", $row->id) . "'><i class=\"fa fa-eye\"></i> View</a>";
                } else {
                    return "<a class='btn blue btn-xs' href='" . route("gym-admin.email-promotion.edit-campaign", $row->id) . "'><i class=\"fa fa-edit\"></i> Edit</a>";
                }
            }
            )
            ->editColumn(
    ...
    {
        if (!$this->data['user']->can("send_promotion")) {
            return App::abort(401);
        }
    ...
    public function sendPromotion(Request $request)
    {
        ...
        switch ($filter) {
            Case 'all':
                $user = MerchantPromotionDatabase::select('id', 'name', 'email', 'mobile', 'age', 'gender')
                    ->where('detail_id', '=', $this->data['user']->detail_id)
                    ->whereIn('id', request()->get('userIds'))
                    ->get();
                break;
            Case 'manual':
                $user = MerchantPromotionDatabase::select('id', 'name', 'email', 'mobile', 'age', 'gender')
        ...
        $message = str_replace('/%%/COPYRIGHT/%%/', Carbon::today('Asia/Kathmandu')->year . '. ' . ucwords($this->data['common_details']->title), $message);

        if ($emailLogo) {
            $message = str_replace(asset('admin/gym-email-template/logo.png'), $emailLogo, $message);
        ...
        if ($res[0]->email_status === 'disabled') {
            return Reply::error('Campaign Not Sent, Enabled your Smtp credentials');
        }
        try {
            foreach ($user as $usr) {
                $email = $usr->email;
            ...
            return Reply::error('Campaign Not Sent, check your Smtp credentials');
        }

        $campaign->sent_on = Carbon::now('Asia/Kathmandu');
