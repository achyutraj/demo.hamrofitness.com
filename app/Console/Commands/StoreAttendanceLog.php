...
class StoreAttendanceLog extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'store:attendance_log';
     ...
     * @var string
     */
    protected $description = 'Store Attendance Log';
    ...
    {
        Log::info('Starting attendance log data second collection');

        $devices = Device::where('device_status',1)->orderBy('id','asc')->skip(50)->take(50)->get();
        ...
        foreach ($devices as $device) {
            try {
                ...
                $logCount = count($logs);
                Log::info( "Processing {$logCount} attendance logs for device: {$device->code}");
