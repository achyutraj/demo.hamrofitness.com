...
class ADMSHelper{
    ...
    public static function storeAttendance($date_range_start,$date_range_end){
        ...
        foreach($devices as $device){
            $clients = getADMSAttendanceLog($date_range_start,$date_range_end,$device->serial_num);
            $logs = self::formattingAttendanceAPIData($clients);
            $attendanceData = [];
    ...
    public static function clearAttendanceLogFromDevice($serial_num){
        $url = self::getBaseUrl() .'api/AdmsEx/ClearAttLogFromDevice?sn=' . $serial_num;
        ...
        try {
            ...
            if ($status == 200) {
                Log::info('Clear Attandance log of device: ' . $serial_num);
                return true;
            } else {
                Log::info('Failed to clear attandance log of device: ' . $serial_num);
                return false;
            }
        } catch (\Exception $e) {
            Log::error('Clear Attandance Log Request timed out or failed: ' . $serial_num . ' , Error: ' . $e->getMessage());
            return null; // Return null if timeout or any error occurs
        }
