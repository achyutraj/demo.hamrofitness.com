<div class="row">
    <div class="col-md-12 attendance-row">
        ...
        <a href="{{route('gym-admin.employAttendance.show',$row->id)}}" class="col-md-2 col-xs-12">
            @if($row->image == '')
        ...
        <div class="search-content col-md-4 col-xs-12">
            <h2>
                <a href="{{route('gym-admin.employAttendance.show',$row->id)}}">{{$row->first_name. ' ' . $row->middle_name. ' ' . $row->last_name}}</a>
            </h2>
        </div>
                ...
                <div class="widget-thumb-wrap">
                    ...
                    <div class="widget-thumb-body">
                        <span class="widget-thumb-body-stat" data-counter="counterup"
                              data-value="{{ $row->total_checkin }}">{{ $row->total_checkin }}</span>
        ...
        <div class="col-md-3 col-xs-12 text-right checkin_btns" id="check-in-btns-{{ $row->id }}">
            @if($row->status != 'arrived')
                <button href="javascript:;" data-client-id="{{ $row->id }}" class="mark-check-in btn-block btn btn-lg green-jungle "><i
                            class="icon-check"></i>
                    Check In
                </button>
            @else
                ...
                @endif
                <button data-delete-id="{{$row->checkin_id}}" data-client-id="{{$row->id}}"
                        class="btn-block delete-button margin-top-10 btn btn-lg red-flamingo"><i class="icon-close"></i>
                    Remove
                </button>
            @endif
            <br>
