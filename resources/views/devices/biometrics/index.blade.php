                    ...
                    <div class="portlet light ">
                        <div class="portlet-title">
                            ...
                            <div class="pull-right">
                                <div class="btn-group">
                                    <a href="{{route('device.biometrics.create')}}" class="btn sbold dark"> Add New
                                        <i class="fa fa-plus"></i>
                                    </a>

                                    <a href="{{route('device.biometrics.addCardForm')}}" class="btn sbold btn-info"> Add Card
                                        <i class="fa fa-plus"></i>
                                    </a>
                                </div>
                            </div>
                        ...
                        <div class="portlet-body">
                            <table class="table table-striped table-bordered table-hover order-column" style="width: 100%"
                                ...
                                <thead>
                                <tr>
                                    <th class="max-desktop"> CardID</th>
...
@section('footer')
    ...
    <script>
        ...
        var table = clientTable.dataTable({
            processing: true,
            serverSide: true,
            ajax: "{{route('device.biometrics.ajax_create')}}",
            columns: [
                {data: 'card', name: 'card'},
    ...
    var UIBootbox = function () {
       var deviceData = function () {
            $('#gym_clients').on('click', '.remove-user', function () {
                ...
                bootbox.confirm({
                    message: "Do you want to delete this client from device?",
                    buttons: {
                        confirm: {
                            label: "Yes",
        ...
        var deniedUserData = function () {
            $('#gym_clients').on('click', '.denied-user', function () {
                var deviceUrl = $(this).data('url');
                bootbox.confirm({
                    message: "Do you want to denied this client from device?",
                    buttons: {
                        confirm: {
                            label: "Yes",
                            className: "btn-primary"
