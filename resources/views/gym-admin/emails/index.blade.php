                                ...
                                <div class="col-md-3">
                                    <div class="inbox-sidebar">
                                        <a href="javascript:;" data-title="Compose"
                                           class="btn red compose-btn btn-block compose-message"
                                           @if ($emailSetting[0]->email_status == 'disabled') disabled @endif>
                                            <i class="fa fa-edit"></i> Compose </a>
                                        @if ($emailSetting[0]->email_status == 'disabled')
                                        <p>Goto settings/email and enable mail status option to send emails</p>
                                        @endif
                                        <ul class="inbox-nav">
                                            <li class="active">
                                                <a href="{{ route('gym-admin.emails.index') }}" data-type="inbox" data-title="Inbox">All Emails
                                                </a>
                                            </li>
                                        </ul>
                                    </div>
...
@section('footer')
    ...
    <script>
        ...
        $('.compose-message').on('click', function () {
            var url = '{{ route('gym-admin.emails.create') }}';
            $.ajaxModal('#emailModal', url);
        });
