                                ...
                                <div class="form-group form-md-line-input ">
                                    <select class="bs-select form-control" data-live-search="true" data-size="8" name="client" id="client">
                                        ...
                                        @foreach($clients as $client)
                                            <option value="{{$client->id}}" @if(isset($reservation) && $reservation->client_id == $client->id) selected @endif >{{$client->fullName}}</option>
                                        @endforeach
                                    </select>
                                    <label for="title">Client Name <span class="required" aria-required="true"> * </span></label>
                                    <span class="help-block"></span>
                                </div>
                                @if(isset($userReservation) && $userReservation)
                                    <div class="form-group form-md-line-input ">
                                        <select class="form-control" name="reservation_id" id="reservation_id">
                                            <option value="{{$reservation->id}}">{{ ucwords($reservation->locker->locker_num) }} - [Purchased on: {{$reservation->purchase_date->format('d-M')}}]</option>
                                        </select>
...
@section('footer')
    ...
    <script>
        ...
        $('#storePayments').on('change', '#reservation_id', function () {
            var price = $('#reservation_id :selected').data('price');
        ...
        $('#client').change(function () {
            ...
            var url = "{{route('gym-admin.reservation-payments.clientPurchases',[':id'])}}";
    ...
    <script>
        $('#save-form').click(function () {
            ...
            $.easyAjax({
                url: "{{route('gym-admin.reservation-payments.store')}}",
