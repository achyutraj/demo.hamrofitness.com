                    ...
                    <div class="portlet light ">
                        <div class="portlet-title">
                            ...
                            <div class="pull-right">
                                <div class="btn-group">
                                    @if ($title === 'active')
                                        <a href="{{ route('gym-admin.client-purchase.inactive') }}" class="btn btn-danger">
                                            Inactive
                                        </a>
                                    @else
                                        <a href="{{ route('gym-admin.client-purchase.active') }}" class="btn btn-success">
                                            Active
                                        </a>
                                    @endif
                                    <a href="{{ route('gym-admin.client-purchase.index') }}" class="btn btn-info">
                                        All Subscriptions
                                    </a>
                                    <a id="addTarget" href="{{ route('gym-admin.client-purchase.create') }}"
                                ...
                                <tbody>
                                    @foreach ($purchases as $key => $purchase)
                                        <tr>
                                            ...
                                            <td class="subscription-cell"><a href="{{ route('gym-admin.client.show', $purchase['client_id']) }}">{{ $purchase['client']['first_name'] }}
                                                    {{ $purchase['client']['middle_name'] }}
                                                    {{ $purchase['client']['last_name'] }}</a></td>
                                            <td class="subscription-cell">{{ $purchase['membership']['title'] }}</td>
                                            <td>NPR {{ $purchase['amount_to_be_paid'] - $purchase['paid_amount'] }}</td>
                                            <td>
                                                @if ($purchase['amount_to_be_paid'] - $purchase['paid_amount'] > 0)
                                                    @if (isset($purchase['next_payment_date']))
                                                        {{ date('M d, Y', strtotime($purchase['next_payment_date'])) }}
                                            ...
                                            <td>
                                                @if ($purchase['status'] == 'extend')
                                                    <span class="label label-info">Extend</span>
                                                @endif
                                                {{ date('M d, Y', strtotime($purchase['expires_on'])) }}
                                                @if (!is_null($purchase['expires_on']) && $purchase['expires_on'] > \Carbon\Carbon::today())
                                                    <label class="label label-success">
                                                        {{ \Carbon\Carbon::parse($purchase['expires_on'])->diffForHumans() }}</label>
                                                @else
                                                ...
                                                @else
                                                    <div class="btn-group"><button class="btn blue btn-xs dropdown-toggle"
                                                        ...
                                                        <ul class="dropdown-menu pull-right" role="menu">
                                                            ...
                                                            @if ($purchase['amount_to_be_paid'] - $purchase['paid_amount'] > 0)
                                                                <li><a
                                                                        href="{{ route('gym-admin.client-purchase.show', $purchase['id']) }}">
                                                                        <i class="fa fa-edit"></i>Edit</a>
                                                                </li>
                                                                <li><a href="javascript:;" data-id="{{ $purchase['id'] }}"
                                                                        class="remove-purchase"> <i
                                                                            class="fa fa-trash"></i>Remove </a>
                                                                </li>
                                                                <li> <a class="add-payment" data-id="{{ $purchase['id'] }}"
                                                            ...
                                                            @endif
                                                            @if ($purchase['membership']['duration_type'] != 'unlimited')
                                                                <li><a class="extend-subscription"
                                                                        data-id="{{ $purchase['id'] }}"
                                                                        href="javascript:;"><i class="fa fa-expand"></i>
                                                                        Extend Subscription</a>
                                                                </li>
                                                                @if ($purchase['expires_on'] > now())
                                                                    @if ($purchase['status'] == 'freeze_pending')
                                                                        <li>
                                                                            <a href=""><i
                                                                                    class="fa fa-pause"></i>Freeze On
                                                                                {{ isset($purchase['freeze_date']) && $purchase['freeze_date'] != null ? date('M d,Y', strtotime($purchase['freeze_date'])) : '' }}</a>
                                                                        </li>
                                                                    @else
                                                            ...
                                                            <li><a class="show-subscription-reminder"
                                                                    data-id="{{ $purchase['id'] }}" href="javascript:;"><i
                                            ...
                                            <td style="display:none;">NPR {{ $purchase['amount_to_be_paid'] }}</td>
                                            <td style="display:none;">
                                                @if ($purchase['is_renew'])
                                                    <span class="label label-info">Renew</span>
                                                @endif
                                                {{ date('M d, Y', strtotime($purchase['start_date'])) }}
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
