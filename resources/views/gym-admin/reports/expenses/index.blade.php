        ...
        <div class="page-content-inner">
            <div class="row">
                <div class="col-md-12">
                    <div class="m-heading-1 border-green m-bordered">
                        ...
                        <p>This report section provides you list of expenses .</p>
                    </div>
                </div>
            </div>
            <div class="portlet light portlet-fit">
                <div class="portlet-title">
                    <div class="caption">
                        <i class="fa fa-dollar font-red"></i><span class="caption-subject font-red bold uppercase">Expense Report</span>
                    </div>
                </div>
                <div class="portlet-body">
                    <div class="row">
                        <div class="col-md-offset-2 col-md-5">
                            {{ html()->form->open(['id'=>'createReport','class'=>'ajax-form']) !!}
                            <div class="form-body">
                                <div class="form-group form-md-line-input">
                                    ...
                                    <label for="title">Select Expense Category</label>
                            ...
                            <div class="row">
                                <div class="col-md-6 col-sm-12" style="padding:10px;">
                                    <div class="caption font-dark">
                                        ...
                                        <span class="caption-subject font-red bold uppercase"> Expense Report</span>
                                    </div>
                                </div>
                                <div class="col-md-6 col-sm-12" style="text-align:right;">
                                    <div class="caption font-dark">
                                        <a class="btn btn-xs btn-success downloadExpense">
                                            <span class="caption-subject font-white bold uppercase"> PDF</span>
                                            <i class="fa fa-download"></i>
                                        </a>
                                        <a class="btn btn-xs btn-info downloadExcelExpense">
...
@section('footer')
    ...
    <script>
        $('#save-form').click(function(){
            ...
            $.easyAjax({
                url: "{{route('gym-admin.reports.expense.store')}}",
                container:'#createReport',
                type:"POST",
                data:{date_range: dateRange,category:category , _token: '{{csrf_token()}}'},
                success:function(res){
                    if(res.status = 'success') {
                        $('#countExpense').attr('data-value',res.total);
                        $('#heading').html(res.report);
                        $('#easyStats').css('display','block');
                        $('#targetDataTable').css('display','block');
                        load_expense_data(res.start_date,res.end_date,res.category);
                        download_expense_data(res.start_date,res.end_date,res.category);
                        $('.counter').counterUp();
                    }
                }
            });
    ...
    <script>
        function load_expense_data(s,e,category){
            var table = $('#expense_table');
            var url = "{{route('gym-admin.reports.expense.ajax_create_expense',['#s','#e','#category'])}}";
            url = url.replace('#s',s);
            url = url.replace('#e',e);
        ...
        function download_expense_data(s,e,category){
            $('.downloadExpense').on('click',function(){
                window.location = 'expense-report/download/'+ s + '/' + e + '/' + category;
            });
            $('.downloadExcelExpense').on('click',function(){
                window.location = 'expense-report/download/excel/'+ s + '/' + e + '/' + category ;
            });
        }
    </script>
@stop
