    ...
    @foreach($tasks as $task)
        ...
        @endif task-{{ $task->id }}" onclick="show({{ $task->id }})" id="todo-container-{{$task->id}}">
            ...
            <div class="todo-tasklist-controls pull-left">
                @if ( $task->deadline  <= \Carbon\Carbon::now()->toFormattedDateString())
                    <span class="todo-tasklist-date fail-alert-date">
                    <i class="fa fa-calendar fail-alert-date"></i> {{ $task->deadline->toFormattedDateString() }} </span>
                @else
                    <span class="todo-tasklist-date success-alert-date">
                    <i class="fa fa-calendar success-alert-date"></i> {{  $task->deadline->toFormattedDateString() }} </span>
                @endif
            </div>
        </div>
    @endforeach
