                    ...
                    <div class="row">
                        ...
                        <div class="col-md-5">
                            <input data-date-format="dd/MM/yyyy" readonly type="text" id="attendance-date"
                                   ...
                                   value="{{ \Carbon\Carbon::now('Asia/Kathmandu')->format('d/M/Y') }}"
...
@section('footer')
        ...
        var dTable = function () {
            ...
            var o = function () {
                ...
                table.dataTable({
                    ...
                    ajax: {
                        url: "{{ route('gym-admin.employ.attendance.ajax_create') }}",
                        data: function (d) {
                    ...
                    "bDestroy": true,
                    "aoColumns": [
                        {data: 'first_name', name: 'first_name','sClass': 'center',"bSearchable": false, },
                    ],
    ...
    <script>
        ...
        $('#checkin_clients').on('click', '.mark-check-in', function () {
            ...
            var show_url = "{{route('gym-admin.employ.attendance.checkin',['#clientId'])}}";
            var url = show_url.replace('#clientId', clientId);
            $('#modelHeading').html('Select Time');
            $.ajaxModal("#attendenceModal", url);
        });

        $('#checkin_clients').on('click', '.mark-check-out', function () {
            var clientId = $(this).data('row-id');
            var show_url = "{{route('gym-admin.employ.attendance.checkout',['#clientId'])}}";
            var url = show_url.replace('#clientId', clientId);
            $('#modelHeading').html('Are you sure want to checkout?');
            $.ajaxModal("#attendenceModal", url);
        });
                    ...
                    bootbox.confirm({
                        ...
                        callback: function (result) {
                            if (result) {
                                ...
                                var url_delete = "{{ route('gym-admin.employAttendance.destroy',['#id']) }}";
